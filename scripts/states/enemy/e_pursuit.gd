...
func process_frame(delta):
	...
	parent.velocity = (next_nav_point - parent.global_position).normalized() * MOVE_SPEED
...
func _on_attack_range_area_entered(area):
	if area is PlayerHitboxComponent && parent.cooldown_timer.is_stopped():
		parent.attack_target = area
